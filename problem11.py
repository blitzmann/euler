...
list = grid.strip().replace("\n", ' ').split(' ')
width = 20
result = 0

def idxVert(i, w, list):
    idxs = [i]
    for x in xrange(1,4):
        t = i-(w*x)
        if t < 0: return False
        idxs.append(t)
    vals = [int(list[i]) for i in idxs]
    product = reduce(lambda x, y: x * y, vals, 1)
    return product
...
def idxDiag(i, w, list):
    idxs = [i]
    for x in xrange(1,4):
        t = i-(w*x+x)
        if (t+1)%w == 0 or t < 0: return False
        idxs.append(t)
    vals = [int(list[i]) for i in idxs]
    product = reduce(lambda x, y: x * y, vals, 1)
    return product

def idxDiag2(i, w, list):
    idxs = [i]
    for x in xrange(1,4):
        t = i-(w*x-x)
        if t%w == 0 or t < 0: return False
        idxs.append(t)
    vals = [int(list[i]) for i in idxs]
    product = reduce(lambda x, y: x * y, vals, 1)
    return product

tests = [idxVert, idxHor, idxDiag, idxDiag2]

for i, x in enumerate(list):
    for func in tests:
        product = func(i, width, list)

        if product and product > result:
            result = product
